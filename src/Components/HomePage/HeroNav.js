        ...
        <>
            <section className="w-full bg-indigo-300">
                <section className="max-w-6xl mx-auto w-full">
                    <nav aria-label="products navigation" className="flex w-1/5 shadow">
                        <ul className="flex flex-col items-start justify-evenly py-1 w-full bg-gray-200">
                            {[                               
                                ...
                                <li key={(item.mainLi.toLowerCase().replace(" ", "").concat(`${Math.ceil(Math.random()*100000)}`))} className={`px-2 py-1 text-sm hover:cursor-pointer hover:bg-gray-300 hover:text-blue-500 w-full flex items-center justify-between relative ${HNavCss.sideNavLi}`} >
                                    <h4>{item.mainLi}</h4>
                                    <div className={HNavCss.icon}><i className="fas fa-chevron-right"></i></div>
                                    {/* <ul className="absolute -right-full hidden bg-white">
